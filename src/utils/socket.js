...
import { createChat, fetchActiveChatsUsers, fetchChatById, fetchMessages, fetchMessagesById, getAdminChatsList, getCallLogs, getChatBetweenUsers, getUserChats, getUserChatsList, saveMessage, uploadMessageFiles } from "../models/chat.js";
...
import { detectUserDataById, get_user_by_id, get_web_user_by_id, getUserDataByRole } from "../models/web_user.js";
    ...
    io.on("connection", async (socket) => {
        try {
            ...
            if (userType == 1) {
                ...
                decoded.userData = await getUserDataByRole(decoded.id, decoded.role);
            ...
            socket.on("send_message", async ({ chatId, message, messageType, files }) => {
                try {
                    let senderId = decoded.id;
                    let ids = [senderId];
                    let fetchChatsUsers = await fetchChatById(chatId);
                    ...
                    await uploadMessageFiles(chatId, messageId, files)
